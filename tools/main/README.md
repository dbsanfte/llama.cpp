...
-   `--numa isolate`: Pin all threads to the NUMA node that the program starts on. This limits the number of cores and amount of memory that can be used, but guarantees all memory access remains local to the NUMA node. When using isolate mode, model data is only allocated on the target NUMA node without mirroring across nodes.
-   `--numa isolate N`: Pin all threads to NUMA node N (where N is a valid NUMA node number). This allows you to specify exactly which NUMA node to use for isolation. If the specified node number is invalid, the system will fall back to using the node where the main thread started.
