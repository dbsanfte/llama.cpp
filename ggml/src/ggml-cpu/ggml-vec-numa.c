...
void ggml_vec_sin_f32_avx512_mvec(int n, float * y, const float * x) {
    ...
    // GNU libmvec with OpenMP SIMD - CONFIRMED WORKING: 1.25x speedup
    #pragma omp simd
    for (int i = 0; i < n; i++) {
        y[i] = sinf(x[i]);
    }
...
void ggml_vec_sincos_f32_avx512_mvec(int n, float * sin_y, float * cos_y, const float * x) {
    ...
    #pragma omp simd
    for (int i = 0; i < n; i++) {
        // Separate calls to prevent GCC from optimizing to unavailable sincosf
        volatile float x_val = x[i];
        sin_y[i] = sinf(x_val);
        cos_y[i] = cosf(x_val);
    }
#else
