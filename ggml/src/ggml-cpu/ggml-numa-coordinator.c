...
extern void test_track_numa_execution(int node_id);
...
static enum ggml_status ggml_numa_node_execute_operation(
    ...
    // Track node-specific execution for debugging
    test_track_numa_execution(coordinator->numa_node);
...
static void * ggml_numa_get_partitioned_work_buffer(size_t required_size_per_thread, int numa_node, int n_threads) {
    ...
    memset(ggml_thread_work_buffer, 0, total_required_size);
