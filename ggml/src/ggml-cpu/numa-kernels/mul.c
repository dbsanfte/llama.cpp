...
/**
 * @brief Query function for MUL kernel - determines optimal execution strategy
 * 
 ...
 */
ggml_numa_execution_strategy_t ggml_numa_kernel_mul_query(const struct ggml_tensor * tensor) {
    // Calculate total elements for strategy selection (hot path - must be fast)
    const size_t total_elements = ggml_nelements(tensor);
    
    // Get cache entry for MUL operation using direct lookup
    ...
    const char* op_name = cache_entry && cache_entry->kernel_name ? cache_entry->kernel_name : "NUMA MUL";
    NUMA_LOG_DEBUG("MUL query: %zu elements -> strategy %d (%s)\n", 
...
/**
 * @brief Calculate work buffer requirements for MUL kernel
 * 
 * Element-wise MUL operation processes data in-place with SIMD optimizations.
 ...
 * @return Work buffer size in bytes (0 for MUL)
 */
size_t ggml_numa_kernel_mul_work_buffer_calc(const struct ggml_tensor * tensor, int total_numa_nodes, int total_threads) {
    // MUL operation is purely element-wise with no intermediate storage
...
/**
 * @brief Register MUL kernel with NUMA system using shared infrastructure
 * 
...
ggml_numa_kernel_registration_info_t ggml_numa_kernel_mul_register(void) {
    ...
    info.work_funcs.single_single_fn = ggml_numa_kernel_mul_execute;
    info.work_funcs.single_multi_fn = ggml_numa_kernel_mul_execute;
    info.work_funcs.data_parallel_fn = ggml_numa_kernel_mul_execute;
    info.work_funcs.valid = true;
    
    // Query and work buffer function pointers (same pattern as ADD)
    info.query_fn = (void*)ggml_numa_kernel_mul_query;
    info.work_buffer_calc_fn = (void*)ggml_numa_kernel_mul_work_buffer_calc;
