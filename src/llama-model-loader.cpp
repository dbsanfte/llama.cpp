...
void llama_model_loader::done_getting_tensors() const {
    if (n_created != n_tensors) {
        // Allow intentional under-population when a layer cap is applied for fast golden tests.
        // We detect this via LLAMINAR_LAYER_CAP. In that scenario we skip creating tensors
        // for truncated layers, so the original GGUF tensor count (n_tensors) will not match
        // the number actually instantiated (n_created). This is safe because those tensors
        // are never referenced during the capped run.
        if (std::getenv("LLAMINAR_LAYER_CAP")) {
            LLAMA_LOG_WARN("%s: tensor count mismatch tolerated due to LLAMINAR_LAYER_CAP (expected %d, created %d)\n",
