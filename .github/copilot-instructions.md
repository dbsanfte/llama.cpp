...
- **Files**: `ggml-numa-openmp-coordinator.c`, `ggml-numa-allocator.c`, `ggml.h`
- **NUMA mirroring**: Instead of `tensor->data` (deprecated), use `tensor_data()`/`tensor_set_data()` macros in `ggml.h` for NUMA-aware access
