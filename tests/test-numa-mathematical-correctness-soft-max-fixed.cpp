                ...
                if (dispatch_result != GGML_STATUS_SUCCESS) {
                    ...
                    if (!ref_ctx) {
                        ...
                        if (!ref_input) {
                            ...
                            struct ggml_tensor* ref_result = ggml_new_tensor_2d(ref_ctx, GGML_TYPE_F32, cols, rows);
                            if (!ref_result) {
                                printf("      ❌ Failed to create reference result tensor for %s\n", size_label);
                            } else {
                                // Set up the result tensor's source pointer for the computation
                                ref_result->src[0] = ref_input;
                                ref_result->src[1] = nullptr; // SOFT_MAX is unary operation
