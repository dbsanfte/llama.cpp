/**
 * NUMA Mathematical Correctness Test: SUB Operation
 * 
 * This test verifies mathematical equivalence between NUMA parallel SUB operations
 * and serial reference implementations. It ensures the NUMA SUB kernel produces
 ...
 *    b) Multi-thread Single-node: Tests subti-threading coordination within single NUMA node  
 ...
 *    - Validates subti-dimensional broadcasting logic (Matrix + Vector patterns)
...
enum TestSizeClass {
    ...
    SMALL,     // Medium tensors for subti-threading tests
...
/**
 * Test Suite Class for SUB Mathematical Correctness
 */
class NumaSubMathematicalCorrectnessTestSuite {
public:
    std::vector<TestResult> results;
    
    /**
     * Test single SUB case with specified dimensions and forced strategy
     */
    bool test_single_SUB_case(int ne0, int ne1, int ne2, int ne3, ggml_numa_execution_strategy_t strategy, 
                             const char* test_name, const char* strategy_name) {
        TEST_PRINTF("\n🧮 Testing SUB %s (%dx%dx%dx%d, strategy=%s)\n", test_name, ne0, ne1, ne2, ne3, strategy_name);
        ...
        // NUMA Test: Execute SUB operation using NUMA executor
        struct ggml_tensor* numa_result = ggml_sub(test_ctx, input_a, input_b);
        if (!numa_result) {
            printf("❌ Failed to create NUMA SUB operation\n");
            ggml_free(test_ctx);
            return false;
        }
        ...
        if (!is_supported) {
            printf("⚠️  SUB operation not supported by NUMA kernels - skipping NUMA test\n");
            ggml_free(test_ctx);
        ...
        // Reference Test: Execute SUB operation using reference implementation
        ...
        struct ggml_tensor* ref_result = ggml_sub(ref_ctx, ref_input_a, ref_input_b);
        ...
        case_passed = compare_float_arrays(numa_data, ref_data, total_elements, "SUB");
        
        if (case_passed) {
            printf("✅ SUB %s test PASSED\n", test_name);
        } else {
            printf("❌ SUB %s test FAILED\n", test_name);
        }
    ...
    /**
     * Test SUB mathematical equivalence using 3-stage approach:
     * 1. Single-thread Single-node: Tests basic functionality and fallback mechanisms
     * 2. Multi-thread Single-node: Tests subti-threading without NUMA complexity  
     ...
     */
    void test_SUB_mathematical_equivalence() {
        TEST_PRINTF("\n🔬 === SUB MATHEMATICAL EQUIVALENCE TESTS ===\n");
        ...
        std::vector<ExecutionStrategy> strategies = {
            ...
            // Strategy 3: Multi-thread, subti-node (data-parallel)
        ...
        for (TestSizeClass size_class : size_classes) {
            for (const auto& strategy_config : strategies) {
                ...
                bool test_passed = test_single_SUB_case(
        ...
        printf("\n📊 SUB Mathematical Equivalence Summary: %d/%d tests passed\n", 
               passed_tests, total_tests);
        
        if (overall_test_passed) {
            printf("✅ All SUB mathematical equivalence tests PASSED\n");
        } else {
            printf("❌ SUB mathematical equivalence tests FAILED: %s\n", failure_reason.c_str());
        }
        
        results.push_back({"SUB_mathematical_equivalence", overall_test_passed, failure_reason});
    }

    /**
     * Test SUB quantization type coverage - comprehensive testing of all supported type combinations
     * 
     * Based on the reference implementation in binary-ops.cpp, SUB supports these type combinations:
     ...
     */
    void test_SUB_quantization_type_coverage() {
        const std::string test_category = "SUB_quantization_type_coverage";
        ...
        TEST_PRINTF("\n🔢 === SUB QUANTIZATION TYPE COVERAGE TESTS ===\n");
        ...
        for (const auto& combo : type_combinations) {
            ...
            if (ctx) {
                ...
                if (src0 && src1) {
                    struct ggml_tensor* result = ggml_sub(ctx, src0, src1);
                    if (result) {
                        // Debug: Show what type ggml_sub actually returns
                        ...
                        if (result->type == combo.dst_type) {
                            ...
                            printf("⚠️  %s: ggml_sub returned %s instead of expected %s\n", 
                        ...
                        printf("❌ %s: Failed to create SUB operation\n", 
                               combo.description);
                        if (failure_reason.empty()) {
        ...
        printf("\n📊 SUB Quantization Coverage Summary: %d/%d type combinations tested\n", 
        ...
        if (success_rate >= 0.95) {  // 95% success rate (allow for a few edge cases)
            printf("✅ SUB quantization coverage PASSED: %d/%d combinations supported (%.1f%%)\n", 
            ...
            printf("❌ SUB quantization coverage FAILED: Only %d/%d combinations supported (%.1f%%)\n", 
                   passed_type_tests, total_type_tests, success_rate * 100);
            all_tests_passed = false;
        }
        
        results.push_back({"SUB_quantization_type_coverage", all_tests_passed, failure_reason});
    }
    
    /**
     * Test SUB broadcasting regression scenarios
     */
    void test_SUB_broadcasting_regression() {
        const std::string test_category = "SUB_broadcasting_regression";
        ...
        TEST_PRINTF("\n🔄 === SUB BROADCASTING REGRESSION TESTS ===\n");
        ...
        if (ctx) {
            ...
            if (matrix && vector) {
                ...
                struct ggml_tensor* result = ggml_sub(ctx, matrix, vector);
                if (result) {
                    ...
                    printf("❌ Failed to create broadcast SUB operation\n");
                    all_tests_passed = false;
                    failure_reason = "Failed to create broadcast SUB operation";
                }
            } else {
        ...
        printf("   This specific pattern was causing memory corruption in NUMA SUB kernel\n");
        ...
        if (corruption_ctx) {
            ...
            if (src0 && src1) {
                ...
                struct ggml_tensor* numa_result = ggml_sub(corruption_ctx, src0, src1);
                if (numa_result) {
                    // Ensure NUMA dispatch is enabled for our test
                    ggml_numa_set_fallback_flag(false);  // Ensure NUMA dispatch is enabled
                    ...
                    printf("   Executing NUMA SUB with data-parallel strategy...\n");
                    ...
                    if (ref_ctx) {
                        ...
                        struct ggml_tensor* ref_result = ggml_sub(ref_ctx, ref_src0, ref_src1);
                    ...
                    printf("   ❌ Failed to create corruption scenario SUB operation\n");
                    all_tests_passed = false;
                    if (failure_reason.empty()) {
                        failure_reason = "Failed to create corruption scenario SUB operation";
                    }
                }
            } else {
        ...
        printf("\n📊 SUB Broadcasting Regression Summary: %s\n", 
               all_tests_passed ? "PASSED" : "FAILED");
        
        results.push_back({"SUB_broadcasting_regression", all_tests_passed, failure_reason});
    }
    
    /**
     * Test SUB threshold regression - ensure small tensors don't get forced into inappropriate strategies
     * 
     ...
     */
    void test_SUB_threshold_regression() {
        TEST_PRINTF("\n🔬 === SUB THRESHOLD REGRESSION TESTS ===\n");
        ...
        std::vector<TestCase> test_cases = {
            // Small tensors that should naturally use single-single or single-subti
        ...
        printf("\n📊 SUB Threshold Regression Summary: %s\n", 
               all_tests_passed ? "PASSED" : "FAILED");
        ...
        results.push_back({"SUB_threshold_regression", all_tests_passed, failure_reason});
    }
    
    /**
     * Test SUB extreme edge cases - ensure ridiculously small tensors work in data-parallel mode
     * 
     ...
     */
    void test_SUB_extreme_edge_cases() {
        TEST_PRINTF("\n🔬 === SUB EXTREME EDGE CASE TESTS ===\n");
        ...
        printf("\n📊 SUB Extreme Edge Case Summary: %s\n", 
               all_tests_passed ? "PASSED" : "FAILED");
        ...
        results.push_back({"SUB_extreme_edge_cases", all_tests_passed, failure_reason});
    }
    ...
    bool test_broadcasting_case(int src0_ne0, int src0_ne1, int src0_ne2, int src0_ne3,
        ...
        struct ggml_tensor* result = ggml_sub(ctx, src0, src1);
        if (!result) {
            printf("   ❌ Failed to create SUB operation for broadcast test\n");
            ggml_free(ctx);
            return false;
        }
     ...
     */
    /**
     * Test for race condition in NUMA SUB data-parallel execution
     * 
     ...
     * output memory sisubtaneously, causing corruption and incorrect results.
     */
    void test_SUB_race_condition_detection() {
        if (!matches_filter("race_condition")) {
            return;
        }
        
        std::string test_category = "SUB_race_condition_detection";
        printf("\n🎯 Testing: %s\n", test_category.c_str());
        ...
        for (int run = 0; run < NUM_RACE_TESTS; ++run) {
            ...
            try {
                ...
                struct ggml_tensor * result = ggml_sub(ctx, src0, src1);
                if (!result) {
                    printf("     ❌ Failed to create SUB operation for run %d\n", run + 1);
                    ggml_free(ctx);
                    overall_success = false;
                    failed_runs++;
                    continue;
                ...
                // Force NUMA execution with subti-threading like the server uses
        ...
        if (overall_success) {
            ...
            printf("     ✅ No race condition detected in NUMA SUB data-parallel execution\n");
    ...
    std::vector<TestResult> run_all_tests() {
        TEST_PRINTF("🚀 Starting NUMA SUB Mathematical Correctness Test Suite\n");
        
        // Initialize NUMA system
        ggml_numa_init(GGML_NUMA_STRATEGY_MIRROR);
        
        // Run all test categories
        test_SUB_mathematical_equivalence();
        test_SUB_quantization_type_coverage();
        test_SUB_broadcasting_regression();
        test_SUB_threshold_regression();  // Test for threshold bug that caused integration test failures
        test_SUB_extreme_edge_cases();    // NEW: Test ridiculously small tensors in data-parallel mode
        test_SUB_race_condition_detection(); // NEW: Critical test to detect data-parallel race conditions
        // Note: Removed invalid real_tensor_precision test - was using ADD arithmetic instead of SUB
...
void show_usage(const char* program_name) {
    ...
    printf("\nFilter Examples:\n");
    printf("  --filter \"MEDIUM.*Multi-thread Multi-node\"  # Run only MEDIUM tensor subti-node tests\n");
    ...
    printf("\nTest Categories:\n");
    printf("  - SUB_mathematical_equivalence: 3-stage execution testing (Single-thread, Multi-thread Single-node, Multi-thread Multi-node)\n");
    printf("  - SUB_quantization_type_coverage: All 7 supported quantization type combinations\n");
    printf("  - SUB_broadcasting_regression: Matrix + Vector broadcasting tests\n");
    printf("\nExecution Stages:\n");
...
int main(int argc, char** argv) {
    ...
    if (!g_summary_only) {
        printf("==================================================================\n");
        printf("🧪 NUMA SUB MATHEMATICAL CORRECTNESS TEST SUITE\n");
        if (g_filter_enabled) {
    ...
    NumaSubMathematicalCorrectnessTestSuite test_suite;
    ...
    if (passed_tests == total_tests) {
        printf("🎉 ALL TESTS PASSED! NUMA SUB kernel is mathematically correct.\n");
        return 0;
    } else {
