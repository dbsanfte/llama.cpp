/**
 * NUMA Mathematical Correctness Test: DIV Operation
 * 
 * This test verifies mathematical equivalence between NUMA parallel DIV operations
 * and serial reference implementations. It ensures the NUMA DIV kernel produces
...
/**
 * Test Suite Class for DIV Mathematical Correctness
 */
...
public:
    ...
    /**
     * Test single DIV case with specified dimensions and forced strategy
     */
    bool test_single_DIV_case(int ne0, int ne1, int ne2, int ne3, ggml_numa_execution_strategy_t strategy, 
                             const char* test_name, const char* strategy_name) {
        TEST_PRINTF("\n🧮 Testing DIV %s (%dx%dx%dx%d, strategy=%s)\n", test_name, ne0, ne1, ne2, ne3, strategy_name);
        ...
        // NUMA Test: Execute DIV operation using NUMA executor
        struct ggml_tensor* numa_result = ggml_div(test_ctx, input_a, input_b);
        if (!numa_result) {
            printf("❌ Failed to create NUMA DIV operation\n");
            ggml_free(test_ctx);
            return false;
        }
        ...
        if (!is_supported) {
            printf("⚠️  DIV operation not supported by NUMA kernels - skipping NUMA test\n");
            ggml_free(test_ctx);
        ...
        // Reference Test: Execute DIV operation using reference implementation
        ...
        struct ggml_tensor* ref_result = ggml_div(ref_ctx, ref_input_a, ref_input_b);
        ...
        case_passed = compare_float_arrays(numa_data, ref_data, total_elements, "DIV");
        
        if (case_passed) {
            printf("✅ DIV %s test PASSED\n", test_name);
        } else {
            printf("❌ DIV %s test FAILED\n", test_name);
        }
    ...
    /**
     * Test DIV mathematical equivalence using 3-stage approach:
     ...
     */
    void test_DIV_mathematical_equivalence() {
        TEST_PRINTF("\n🔬 === DIV MATHEMATICAL EQUIVALENCE TESTS ===\n");
        ...
        for (TestSizeClass size_class : size_classes) {
            for (const auto& strategy_config : strategies) {
                ...
                bool test_passed = test_single_DIV_case(
        ...
        printf("\n📊 DIV Mathematical Equivalence Summary: %d/%d tests passed\n", 
               passed_tests, total_tests);
        
        if (overall_test_passed) {
            printf("✅ All DIV mathematical equivalence tests PASSED\n");
        } else {
            printf("❌ DIV mathematical equivalence tests FAILED: %s\n", failure_reason.c_str());
        }
        
        results.push_back({"DIV_mathematical_equivalence", overall_test_passed, failure_reason});
    }

    /**
     * Test DIV quantization type coverage - comprehensive testing of all supported type combinations
     * 
     * Based on the reference implementation in binary-ops.cpp, DIV supports these type combinations:
     ...
     */
    void test_DIV_quantization_type_coverage() {
        const std::string test_category = "DIV_quantization_type_coverage";
        ...
        TEST_PRINTF("\n🔢 === DIV QUANTIZATION TYPE COVERAGE TESTS ===\n");
        ...
        for (const auto& combo : type_combinations) {
            ...
            if (ctx) {
                ...
                if (src0 && src1) {
                    struct ggml_tensor* result = ggml_div(ctx, src0, src1);
                    if (result) {
                        // Debug: Show what type ggml_div actually returns
                        ...
                        if (result->type == combo.dst_type) {
                            ...
                            printf("⚠️  %s: ggml_div returned %s instead of expected %s\n", 
                        ...
                        printf("❌ %s: Failed to create DIV operation\n", 
                               combo.description);
                        if (failure_reason.empty()) {
        ...
        printf("\n📊 DIV Quantization Coverage Summary: %d/%d type combinations tested\n", 
        ...
        if (success_rate >= 0.95) {  // 95% success rate (allow for a few edge cases)
            printf("✅ DIV quantization coverage PASSED: %d/%d combinations supported (%.1f%%)\n", 
            ...
            printf("❌ DIV quantization coverage FAILED: Only %d/%d combinations supported (%.1f%%)\n", 
                   passed_type_tests, total_type_tests, success_rate * 100);
            all_tests_passed = false;
        }
        
        results.push_back({"DIV_quantization_type_coverage", all_tests_passed, failure_reason});
    }
    
    /**
     * Test DIV broadcasting regression scenarios
     */
    void test_DIV_broadcasting_regression() {
        const std::string test_category = "DIV_broadcasting_regression";
        ...
        TEST_PRINTF("\n🔄 === DIV BROADCASTING REGRESSION TESTS ===\n");
        ...
        if (ctx) {
            ...
            if (matrix && vector) {
                ...
                struct ggml_tensor* result = ggml_div(ctx, matrix, vector);
                if (result) {
                    ...
                    printf("❌ Failed to create broadcast DIV operation\n");
                    all_tests_passed = false;
                    failure_reason = "Failed to create broadcast DIV operation";
                }
            } else {
        ...
        printf("   This specific pattern was causing memory corruption in NUMA DIV kernel\n");
        ...
        if (corruption_ctx) {
            ...
            if (src0 && src1) {
                ...
                struct ggml_tensor* numa_result = ggml_div(corruption_ctx, src0, src1);
                if (numa_result) {
                    // Ensure NUMA dispatch is enabled for our test
                    ggml_numa_set_fallback_flag(false);  // Ensure NUMA dispatch is enabled
                    ...
                    printf("   Executing NUMA DIV with data-parallel strategy...\n");
                    ...
                    if (ref_ctx) {
                        ...
                        struct ggml_tensor* ref_result = ggml_div(ref_ctx, ref_src0, ref_src1);
                    ...
                    printf("   ❌ Failed to create corruption scenario DIV operation\n");
                    all_tests_passed = false;
                    if (failure_reason.empty()) {
                        failure_reason = "Failed to create corruption scenario DIV operation";
                    }
                }
            } else {
        ...
        printf("\n📊 DIV Broadcasting Regression Summary: %s\n", 
               all_tests_passed ? "PASSED" : "FAILED");
        
        results.push_back({"DIV_broadcasting_regression", all_tests_passed, failure_reason});
    }
    
    /**
     * Test DIV threshold regression - ensure small tensors don't get forced into inappropriate strategies
     * 
     ...
     */
    void test_DIV_threshold_regression() {
        TEST_PRINTF("\n🔬 === DIV THRESHOLD REGRESSION TESTS ===\n");
        ...
        printf("\n📊 DIV Threshold Regression Summary: %s\n", 
               all_tests_passed ? "PASSED" : "FAILED");
        ...
        results.push_back({"DIV_threshold_regression", all_tests_passed, failure_reason});
    }
    
    /**
     * Test DIV extreme edge cases - ensure ridiculously small tensors work in data-parallel mode
     * 
     ...
     */
    void test_DIV_extreme_edge_cases() {
        TEST_PRINTF("\n🔬 === DIV EXTREME EDGE CASE TESTS ===\n");
        ...
        printf("\n📊 DIV Extreme Edge Case Summary: %s\n", 
               all_tests_passed ? "PASSED" : "FAILED");
        ...
        results.push_back({"DIV_extreme_edge_cases", all_tests_passed, failure_reason});
    }
    ...
    bool test_broadcasting_case(int src0_ne0, int src0_ne1, int src0_ne2, int src0_ne3,
        ...
        struct ggml_tensor* result = ggml_div(ctx, src0, src1);
        if (!result) {
            printf("   ❌ Failed to create DIV operation for broadcast test\n");
            ggml_free(ctx);
            return false;
        }
     ...
     */
    /**
     * Test for race condition in NUMA DIV data-parallel execution
     * 
     ...
     */
    void test_DIV_race_condition_detection() {
        if (!matches_filter("race_condition")) {
            return;
        }
        
        std::string test_category = "DIV_race_condition_detection";
        printf("\n🎯 Testing: %s\n", test_category.c_str());
        ...
        for (int run = 0; run < NUM_RACE_TESTS; ++run) {
            ...
            try {
                ...
                struct ggml_tensor * result = ggml_div(ctx, src0, src1);
                if (!result) {
                    printf("     ❌ Failed to create DIV operation for run %d\n", run + 1);
                    ggml_free(ctx);
                    overall_success = false;
                    failed_runs++;
                    continue;
        ...
        if (overall_success) {
            ...
            printf("     ✅ No race condition detected in NUMA DIV data-parallel execution\n");
    ...
    std::vector<TestResult> run_all_tests() {
        TEST_PRINTF("🚀 Starting NUMA DIV Mathematical Correctness Test Suite\n");
        
        // Initialize NUMA system
        ggml_numa_init(GGML_NUMA_STRATEGY_MIRROR);
        
        // Run all test categories
        test_DIV_mathematical_equivalence();
        test_DIV_quantization_type_coverage();
        test_DIV_broadcasting_regression();
        test_DIV_threshold_regression();  // Test for threshold bug that caused integration test failures
        test_DIV_extreme_edge_cases();    // NEW: Test ridiculously small tensors in data-parallel mode
        test_DIV_race_condition_detection(); // NEW: Critical test to detect data-parallel race conditions
        // Note: Removed invalid real_tensor_precision test - was using ADD arithmetic instead of DIV
...
void show_usage(const char* program_name) {
    ...
    printf("\nTest Categories:\n");
    printf("  - DIV_mathematical_equivalence: 3-stage execution testing (Single-thread, Multi-thread Single-node, Multi-thread Multi-node)\n");
    printf("  - DIV_quantization_type_coverage: All 7 supported quantization type combinations\n");
    printf("  - DIV_broadcasting_regression: Matrix + Vector broadcasting tests\n");
    printf("\nExecution Stages:\n");
...
int main(int argc, char** argv) {
    ...
    if (!g_summary_only) {
        printf("==================================================================\n");
        printf("🧪 NUMA DIV MATHEMATICAL CORRECTNESS TEST SUITE\n");
        if (g_filter_enabled) {
    ...
    if (passed_tests == total_tests) {
        printf("🎉 ALL TESTS PASSED! NUMA DIV kernel is mathematically correct.\n");
        return 0;
    } else {
